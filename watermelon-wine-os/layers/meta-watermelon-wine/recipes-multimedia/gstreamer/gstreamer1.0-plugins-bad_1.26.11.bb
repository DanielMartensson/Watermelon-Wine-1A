...
SRC_URI = "https://gstreamer.freedesktop.org/src/gst-plugins-bad/gst-plugins-bad-${PV}.tar.xz \
           file://gstreamer-webrtcbin-audio-opus-ptmap-fallback.patch \
           file://gstreamer-webrtcbin-balanced-to-maxbundle.patch \
           "
SRC_URI[sha256sum] = "110fb82795f0e569b1e27b12ab9699d35c7762e1ff4db95335d6ac8d1442af3d"
...
# ImWebBrowser: webrtcbin fixes required for GeForce Now (applied from source)
#   - audio-opus-ptmap-fallback: advertise an Opus receive-only m-line when the
#     remote peer only sends audio (no local caps to derive a codec from), so
#     the SDP answer keeps the audio m-line and incoming pt-111 packets aren't
#     dropped -> peer audio flows and the session isn't aborted.
#   - balanced-to-maxbundle: map the unimplemented BALANCED bundle policy to
#     MAX_BUNDLE instead of erroring, which some WebRTC peers request.
...
PACKAGECONFIG ??= " \
    ...
    sndfile ttml uvch264 webp webrtc sctp srtp \
